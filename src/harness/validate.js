...
function measure () {
  ...
  document.querySelectorAll('body *').forEach(el => {
    ...
    const cs = getComputedStyle(el); const fs = parseFloat(cs.fontSize); mN++
    if (fs + 0.5 < minFont) mF.push(fs.toFixed(1) + 'px [' + label(el) + ']')
    const fg = parseRGB(cs.color); if (!fg) return; cN++
    const r = ratio(fg, bgOf(el)); const bold = (parseInt(cs.fontWeight) || 400) >= 700
  ...
  if (wx && rec) {
    const page = rec.closest('.page') || document.body; const pr = page.getBoundingClientRect(); const rr = rec.getBoundingClientRect()
    ...
    const ox = rr.left - pr.left; const oy = rr.top - pr.top
